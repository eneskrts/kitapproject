    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not user.is_active:
                ...
                if not kullanici:
                    messages.success(request, "Bu bilgilerde kullanıcı bulunamadı", extra_tags="danger")
                    return render(request, "kullanici/login.html", context={'form': form})
...
def ProfileView(request):
    if request.method == "GET":#update view
        ...
        initial = {'first_name': user.first_name, 'last_name': user.last_name,
                   'email': user.email, 'phone': user.phone,'il':user.il.replace("i","İ").upper(),'ilce':user.ilce.replace("i","İ").upper(),'adres':user.adres}
